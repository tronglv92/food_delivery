...
func (rabbit *appRabbitMQ) Publish(done chan bool, exchangeName string, routingKey string, data *pubsub.Message) error {
	...
	rabbit.logger.Infof("got Channel, declaring %q Exchange (%q)", rabbit.exchangeType, exchangeName)
	if err := channel.ExchangeDeclare(
		exchangeName,        // name
		rabbit.exchangeType, // type
		true,                // durable
		false,               // auto-deleted
		false,               // internal
		false,               // noWait
	...
	if rabbit.reliable {
		...
		go rabbit.confirmHandler(done, publishes, confirms)
	}
...
func (rabbit *appRabbitMQ) Subscribe(done chan error, exchangeName string, queueName string, routingKey string) (ch <-chan *pubsub.Message, close func()) {
	...
	go rabbit.handle(msgChan, deliveries, done)
	return msgChan, func() {
		connection.Close()
	}
}
