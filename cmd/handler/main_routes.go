...
func MainRoute(router *gin.Engine, sc goservice.ServiceContext) {
	...
	v1 := router.Group("/v1")
	{
		...
		{
			restaurants.POST("", ginrestaurant.CreateRestaurant(sc))
			restaurants.DELETE("/:id", ginrestaurant.DeleteRestaurant(sc))
			restaurants.GET("", ginrestaurant.ListRestaurant(sc))
			// restaurants.POST("/:id/like", ginrstlike.UserLikeRestaurant(sc))
			// restaurants.DELETE("/:id/dislike", ginrstlike.UserDislikeRestaurant(sc))
			// restaurants.GET("/:id/like", ginrstlike.ListUser(sc))
		}
	}
}
