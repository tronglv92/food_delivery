...
func LoginFacebook(sc goservice.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		loginFbStorage := userstoreremote.NewUserRestfulStore(client)
		userStorage := userstore.NewSQLStore(db)
		...
		repo := userrepo.NewLoginFacebookRepo(loginFbStorage, userStorage, tokenProvider, common.AccessTokenDuration, common.RefreshTokenDuration, md5, storeRedis, storeSession)
		account, err := repo.LoginFacebook(c.Request.Context(), request.AccessToken, c.Request.UserAgent(), c.ClientIP())
